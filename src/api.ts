...
const handleKeyboardEvent = (
	...
	const modifier =
		KEYBOARD_MODIFIER_MAP[event.key as keyof typeof KEYBOARD_MODIFIER_MAP];
	if (modifier) {
...
function setupMouseIO(canvas: HTMLCanvasElement) {
	...
	canvas.addEventListener("mousedown", (e) => {
		io.AddMouseButtonEvent(
			MOUSE_BUTTON_MAP[e.button as keyof typeof MOUSE_BUTTON_MAP],
			true,
		);
	});
...
export const ImGuiImplWeb = {
	...
	async Init(options: InitOptions) {
		...
		if (backendName === "webgl" || backendName === "webgl2") {
			...
			if (!ctx)
				throw new Error(
					`Failed to create ${backendName.toUpperCase()} context`,
				);
