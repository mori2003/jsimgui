...
EMSCRIPTEN_BINDINGS(generated) {
    ...
    bind_func("ImGui_ColorPicker4", [](std::string label, emscripten::val col, ImGuiColorEditFlags flags, emscripten::val ref_col){
        auto col_bind = ArrayParam<float>(col);
        auto ref_col_bind = ArrayParam<const float>(ref_col);
        const auto ret = ImGui_ColorPicker4(label.c_str(), &col_bind, flags, &ref_col_bind);
        return ret;
    }, allow_ptr());
